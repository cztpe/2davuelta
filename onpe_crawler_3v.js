 ...
 *   y mesas instaladas / no instaladas (mesa/totales).
 *
...
(async () => {
  ...
  console.log("2) capturando totales + candidatos + mesas (pool " + POOL + ")...");
  const data = {}; let master = null;
  await pool(toFetch, POOL, async n => {
    const [tot, cand, mesa] = await Promise.all([
      jget(totalesPath(n)),
      jget(candsPath(n)),
      jget(mesaPath(n)).catch(() => ({ data: {} }))   // mesa opcional: si falla, no rompe el nodo
    ]);
