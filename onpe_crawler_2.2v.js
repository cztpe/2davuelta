 ...
 *
 * Por que en el navegador: usa tu sesion real y los headers que el proxy
 * de ONPE exige (Content-Type: application/json en cada GET; sin ese header
 * el servidor devuelve el HTML de la pagina en vez de JSON). No necesita
 * instalar nada. Al terminar descarga un snapshot listo para snapshots/.
 *
 * Captura por nodo (hasta distrito en Peru, ciudad en extranjero):
 *   avance, participacion, actas contabilizadas/total, votos por candidato,
 *   emitidos/validos, desglose JEE (enviadasJee = observadas / pendientesJee),
 *   y mesas instaladas / no instaladas (mesa/totales, solo a nivel nacional y departamento).
 *
 * Uso: pegar todo y Enter. No tocar la pestania mientras corre (~minutos).
 * El resultado queda tambien en window.__SNAP.
 */
(async () => {
  const ID = 10, BASE = "/presentacion-backend", POOL = 12;  // gentil: pegarle muy fuerte a un server sobrecargado lo hace fallar mas
  const LR = "TODOS,PERÚ,EXTRANJERO";
  const HEADERS = { Accept: "*/*", "Content-Type": "application/json" }; // <- clave: el proxy exige Content-Type json
  const sleep = ms => new Promise(r => setTimeout(r, ms));

  async function jget(path, tries = 2) {  // fail-fast: 2 intentos en vez de 4
    let last;
    for (let t = 0; t < tries; t++) {
      try {
        ...
        if (!ct.includes("json")) throw new Error("no-JSON (" + ct + ")");
        return await r.json();
      } catch (e) { last = e; await sleep(200 * (t + 1) + Math.random() * 200); }
    }
    throw last;
  }
  ...
  function mesaPath(n) {
    if (n.level === 0) return "/mesa/totales" + qs({ tipoFiltro: "eleccion" });
    const o = { idAmbitoGeografico: n.ambito, idEleccion: ID, tipoFiltro: "ubigeo_nivel_0" + n.level, idUbigeoDepartamento: n.dep };
    if (n.level >= 2) o.idUbigeoProvincia = n.prov;
    if (n.level >= 3) o.idUbigeoDistrito = n.dist;
    return "/mesa/totales" + qs(o);
  }
  ...
  console.log("2) capturando totales + candidatos (+ mesas en dep.) - pool " + POOL + "...");
  const data = {}; let master = null;
  async function capture(n) {
    const [tot, cand, mesa] = await Promise.all([
      jget(totalesPath(n)),
      jget(candsPath(n)),
      n.level <= 1 ? jget(mesaPath(n)).catch(() => ({ data: {} })) : Promise.resolve({ data: {} })  // mesas solo nacional+departamento
    ]);
    const t = tot.data || {}, list = cand.data || [], mz = mesa.data || {}, votes = {};
    ...
    data[n.key] = {
      ...
      mesasInstaladas: mz.mesasInstaladas, mesasNoInstaladas: mz.mesasNoInstaladas
    };
  ...
  await pool(toFetch, POOL, capture);
  // pasada(s) de recuperacion: reintenta SOLO los nodos caidos, con pausa entre rondas para darle aire al server
  for (let round = 1; round <= 6; round++) {
    const missing = toFetch.filter(n => !data[n.key] || data[n.key].cont == null);
    if (!missing.length) break;
    console.log("   recuperacion " + round + "/6: reintentando " + missing.length + " nodos faltantes...");
    await sleep(2500);
    await pool(missing, Math.max(6, Math.floor(POOL / 2)), capture);  // recuperacion aun mas gentil
  }
  {
    const got = toFetch.filter(n => data[n.key] && data[n.key].cont != null).length;
    console.log("   capturados " + got + " / " + toFetch.length + (got < toFetch.length ? "  (OJO: faltan nodos, ONPE inestable; conviene recorrer de nuevo)" : "  (completo)"));
  }
  ...
  if (root.enviadasJee != null) console.log("   jee: enviadas " + root.enviadasJee + " / pendientes " + root.pendientesJee);
  if (root.mesasNoInstaladas != null) console.log("   mesas: instaladas " + root.mesasInstaladas + " / NO instaladas " + root.mesasNoInstaladas);
